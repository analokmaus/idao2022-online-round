...
from .callbacks import ModelCheckpointMAE, ManualStop, ReduceLRUponNan
...
class GraphModel:
    ...
    def __init__(
        ...
        metadata: Dict = None,
        metrics: Union[str, Callable] = None, # For model selection
        metrics_mode: str = 'max',
        log_metrics: Union[str, Callable] = None, 
        **kwargs,
    ):
        """
        Args:
        ...
        **kwargs,
        ...
        if (validation_graphs is not None) and (validation_targets is not None):
            if filename is None:
                ...
                filepath = os.path.join(dirname, f"{filename}.hdf5")
            ...
            if save_checkpoint:
                callbacks.extend(
                    [
                        ModelCheckpointMAE(
                            filepath=filepath,
                            monitor=monitor if self.metrics is None else self.metrics,
                            mode=mode if self.metrics is None else self.metrics_mode,
                            log_monitor=self.log_metrics, 
                            save_best_only=True,
