...
def mae_ohem(target, approx, p=0.5):
    '''
    Online Hard Example Mining
    '''
    bs = tf.cast(len(target), tf.float16)
    n = tf.cast(bs*p, tf.int32)
    mae = tf.math.abs(target - approx)
    arg = tf.argsort(mae)
    loss = tf.reduce_mean(mae[arg >= n])
    return loss


def threshold_mae(target, approx, threshold=0.01):
    mae = tf.math.abs(target - approx)
    loss = tf.reduce_mean(mae[mae > threshold])
    return loss


def smooth_threshold_mae(target, approx, threshold=0.01):
    mae = tf.math.abs(target - approx) - threshold
    loss = tf.reduce_mean(mae[mae > 0])
    return loss
