...
from megnet_models import MEGNetModel, SGDRScheduler
...
if __name__ == '__main__':
    ...
    EXTERNAL = None
    PSEUDO_LABELS = pd.read_csv('results/exp_19_b/submission.csv')
    ...
    pseudo_labels = test_predictions.merge(PSEUDO_LABELS, on='id', how='left')
    ...
    if not args.inference: # Training and inference
        ...
        res = model.train(
            train_all['structure'].tolist() + test_structures,
            train_all['band_gap'].tolist() + pseudo_labels['predictions'].tolist(),
            validation_structures=train_all['structure'],
            validation_targets=train_all['band_gap'],
            epochs=MAX_EPOCHS,
            ...
            callbacks=[
                SGDRScheduler(
                    max_lr=LR, min_lr=LR/20, lr_decay=0.9, cycle_length=10, mult_factor=1.5),
            ],
        )
        results = pd.DataFrame(loadfn(OUTPUT_DIR/f'checkpoint.json'))
        results['trn_loss'] = res.history.history['loss']
        results['trn_energy_within_threshold'] = res.history.history['energy_within_threshold']
        results[
            ['epoch', 'best_epoch', 
            'trn_loss', 'trn_energy_within_threshold', 
            'val_mae', 'val_energy_within_threshold', 'best_energy_within_threshold']].to_csv(
                OUTPUT_DIR/'results.csv', float_format="%10.5f", index=False)
